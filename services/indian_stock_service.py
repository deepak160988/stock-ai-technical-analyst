...
class IndianStockService:
    ...
    def get_indian_stock_historical_data(self, symbol: str, days: int = 365, timeframe: Optional[str] = None) -> pd.DataFrame:
        ...
        Args:
            symbol: Stock symbol
            days: Number of days (used if timeframe is not specified)
            timeframe: Timeframe string (e.g., '1d', '1mo', '1y'). If provided, overrides days parameter.
        ...
        try:
            ...
            # Determine period and interval based on timeframe or days
            if timeframe:
                # Use timeframe mapper for period/interval
                params = map_timeframe(timeframe)
                period = params['period']
                interval = params['interval']
                cache_key = f"{nse_symbol}_{timeframe}"
            else:
                # Fallback to legacy days-based approach
                period = f"{days}d"
                interval = None
            ...
            logger.info(f"Retrieved {len(df)} rows of data for {nse_symbol} (period={period}, interval={interval or 'default'})")
            return df
